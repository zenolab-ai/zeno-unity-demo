...
using static ZenoSDK.Options.NavigationOptions;
...
public class ExampleGame : MonoBehaviour
{
    ...
    {
        if (!AnchoredZone.IsAvailable() || AnchoredZone.Get().GetNavigationOptions().navigationMode != NavigationMode.Unity)
        {
    ...
    public void OnNavButtonToggled(Toggle toggle)
    {
        ...
        // Toggle navigation.
        var navOptions = AnchoredZone.Get().GetNavigationOptions();
        if (toggle.isOn)
        {
    ...
    private void Update()
    {
        ...
        var pose = AnchoredZone.Get().GetCurrentCursorPose();
        if (pose.HasValue && AnchoredZone.Get().GetNavigationOptions().navigationMode == NavigationMode.Unity)
        {
